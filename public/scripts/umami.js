!(function () {
  ...
  (t => {
    const {
      ...
      { hostname: s, href: o } = n,
      { currentScript: u, referrer: l } = i,
      d = o.startsWith("data:") ? void 0 : t.localStorage;
    if (!u) return;
    const m = "data-",
      f = u.getAttribute.bind(u),
      h = f(m + "website-id"),
      p = f(m + "host-url"),
      g = f(m + "tag"),
      ...
      S = `${(p || "https://api-gateway.umami.dev" || u.src.split("/").slice(0, -1).join("/")).replace(/\/$/, "")}/api/send`,
      N = `${e}x${a}`,
      ...
      E = () =>
        !h || (d && d.getItem("umami.disabled")) || (v && !w.includes(s)),
      L = async (t, e = "event") => {
        if (E()) return;
      ...
      $ = () => {
        B ||
          ...
          (() => {
            ...
            ((c.pushState = t(c, "pushState", k)),
              (c.replaceState = t(c, "replaceState", k)));
          })(),
          (() => {
    ...
    y &&
      ...
      ("complete" === i.readyState
        ? $()
        : i.addEventListener("readystatechange", $, !0));
  })(window);
})();
